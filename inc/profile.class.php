...
class PluginVehiculesProfile extends CommonDBTM {
   
   static function getTypeName($nb=0) {
      return __('Rights management', 'vehicules');
   }
   ...
   function getTabNameForItem(CommonGLPI $item, $withtemplate=0) {
      ...
      if ($item->getType()=='Profile') {
            return PluginVehiculesVehicule::getTypeName(2);
      }
      return '';
   }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum=1, $withtemplate=0) {
      ...
      if ($item->getType()=='Profile') {
         ...
         $prof->showForm($item->getField('id'), array('target' => 
                           $CFG_GLPI["root_doc"]."/plugins/vehicules/front/profile.form.php"));
      }
      return true;
   }
   ...
   static function changeProfile() {
      ...
      if ($prof->getFromDBByProfile($_SESSION['glpiactiveprofile']['id'])) {
         $_SESSION["glpi_plugin_vehicules_profile"]=$prof->fields;
      } else {
         unset($_SESSION["glpi_plugin_vehicules_profile"]);
      }
   }
	...
	function showForm ($ID, $options=array()) {
      ...
      $this->showFormHeader($options);
		...
		echo "<th colspan='4'>" . sprintf(__('%1$s - %2$s'), __('Rights management', 'vehicules'),
         $prof->fields["name"])."</th>";
      
      echo "</tr>";
		echo "<tr class='tab_bg_2'>";
		
		echo "<td>".PluginVehiculesVehicule::getTypeName(2)."</td><td>";

		if ($prof->fields['interface']!='helpdesk') {
			Profile::dropdownNoneReadWrite("vehicules",$this->fields["vehicules"],1,1,1);
		} else {
			_e('No access'); // No access;
		}
		echo "</td>";

		echo "<td>" . __('Associable items to a ticket') . " - " . PluginVehiculesVehicule::getTypeName(2) . "</td><td>";
